   ...
   // }
    ...
    public function nodeqrcode($request, $response, $args)
    {
        ...
        if ($node == null) {
            ...
            if ($ctype == "SSR") {
                $ssurl = $ary['server']. ":" . $ary['server_port'].":".str_replace("_compatible","",$ary['protocol']).":".$ary['method'].":".str_replace("_compatible","",$ary['obfs']).":".Tools::base64_nopadding_url_encode($ary['password'])."/?&remarks=".Tools::base64_nopadding_url_encode($node->name);
                $ssqr = "ssr://" . Tools::base64_nopadding_url_encode($ssurl);
            }elseif($ctype == "SS"){
